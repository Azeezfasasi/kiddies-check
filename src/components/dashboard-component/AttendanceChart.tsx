  ...
  const fetchAttendanceData = async (sid) => {
    try {
      ...
      if (res.ok) {
        ...
        const breakdownByClass = records.reduce((acc: Record<string, Loose>, record) => {
        ...
        const classBreakdownData = Object.values(breakdownByClass as Record<string, Loose>)
          .map((item): Loose => ({
            ...item,
            ...
            <ResponsiveContainer width="100%" height={320}>
              <PieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={({ percentage }: Loose) => `${percentage}%`}
                  outerRadius={100}
                  innerRadius={60}
                  fill="#8884d8"
                  dataKey="value"
