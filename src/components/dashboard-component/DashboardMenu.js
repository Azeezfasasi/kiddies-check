...
export default function DashboardMenu({ collapsed, mobileOpen = false, onClose = () => { } }) {
  ...
  const desktopNav = (
    <nav className={`hidden lg:flex h-screen bg-blue-900 border-r overflow-y-auto border-gray-100 ${collapsed ? 'w-16' : 'w-75'} transition-width duration-200`} aria-label="Dashboard navigation">
      ...
      <nav className="relative z-50 h-full w-[80%] bg-blue-900 border-r border-gray-100">
        <div className="h-full overflow-y-auto py-6 px-4">
          <div className="flex items-center justify-between mb-6">
            <Link href="/" className="flex flex-col items-center gap-3 pt-6">
              <Image src="/kiddieschecklogo.png" alt="Kiddies Check Logo" width={170} height={50} priority className="w-35 block rounded-md" />
            </Link>
