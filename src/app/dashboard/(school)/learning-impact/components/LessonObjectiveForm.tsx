...
export default function LessonObjectiveForm({ onSubmit, onCancel, editingItem, teachers, classes, subjects }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} className="space-y-4">
      <AccordionSection title="Lesson details" open>
      ...
      <AccordionSection title="Objectives" open>
        <div>
            ...
            <div key={index} className="bg-gray-50 p-3 rounded-lg mb-2 space-y-2">
              <div className="flex flex-col md:flex-row gap-2">
                <input
                  ...
                  required
                />
                <select value={obj.rating} onChange={(e) => updateObjective(index, "rating", e.target.value)} className="w-full md:w-20 px-3 py-2 border rounded-lg">
      ...
      <AccordionSection title="Summary" open>
