  ...
  useEffect(() => {
    ...
    const loadSchools = async () => {
      if (role === "admin") {
        ...
        const ownId =
          ...
          ((user?.schoolId as { _id?: string } | undefined)?._id || user?.schoolId || "").toString();
        if (!ownId) return;
        setSchoolId(ownId);
    ...
    const loadCalendar = async () => {
      ...
      if (storedValid) {
        ...
        const latest = [...terms].sort((a, b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime())[0];
