...
export default function TeacherRatingForm({ onSubmit, onCancel, editingItem, teachers, classes, subjects }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }} className="space-y-4">
      <AccordionSection title="Teacher & lesson info" open>
      ...
      <AccordionSection title="Rating dimensions" open>
        <div>
          {formData.dimensions.map((dim, index) => (
            <div key={index} className="flex flex-col gap-2 mb-2 rounded-lg border border-gray-200 bg-gray-50 p-3 sm:flex-row sm:items-center">
              <span className="flex-1 text-sm font-medium">{TEACHER_DIMENSIONS.find((d) => d.key === dim.dimension)?.label || dim.dimension}</span>
              <select value={dim.score} onChange={(e) => updateDimension(index, "score", e.target.value)} className="w-full sm:w-20 px-3 py-2 border rounded-lg">
      ...
      <AccordionSection title="Performance summary" open>
