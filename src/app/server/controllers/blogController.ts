      ...
      if (key === 'blogImages' || key === 'featuredImage') {
        if (value instanceof File) {
          ...
          if (key === 'featuredImage') {
            blogData[key] = (uploadResult as { secure_url: string }).secure_url;
          } else if (key === 'blogImages') {
            if (!blogData[key]) blogData[key] = [];
            blogData[key].push((uploadResult as { secure_url: string }).secure_url);
          }
        }
      } else {
        blogData[key] = value;
...
export async function deleteComment(req, { params }) {
  ...
  try {
    ...
    (blog as Loose).comments = blog.comments.filter(comment => comment._id.toString() !== commentId);
    await blog.save();
