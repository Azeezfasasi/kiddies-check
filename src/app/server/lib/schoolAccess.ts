...
export async function canAccessSchool(user: SchoolScopedUser | null | undefined, schoolId: string): Promise<boolean> {
  if (!user) return false;
  // Platform support roles with CBT / report-card access work across all schools.
  if (can(user.role, "cbt") || can(user.role, "report-cards")) return true;
