...
import StudentBill, { computeBillTotals, type StudentBillDocument } from "@/app/server/models/StudentBill";
import type { FeeStructureDocument } from "@/app/server/models/FeeStructure";
...
export const PAYMENT_METHODS = ["cash", "bank-transfer", "pos", "cheque", "online", "other"] as const;

export type Term = (typeof TERMS)[number];
export type PaymentMethod = (typeof PAYMENT_METHODS)[number];
/** A MongoDB id as it arrives from a request or a document. */
export type Id = string | mongoose.Types.ObjectId;
type ObjectId = mongoose.Types.ObjectId;
type UserLike = { role?: string | null } | null | undefined;

export type AuthFailure = { error: string; status: number; user?: undefined };
export type AuthSuccess = { user: UserDocument; error?: undefined; status?: undefined };
export type AuthResult = AuthSuccess | AuthFailure;

export interface SchoolAccessOptions {
  requireFeeManager?: boolean;
...
export const canViewBilling = (user: UserLike): boolean => BILLING_ROLES.includes(user?.role) || can(user?.role, "billing", "view");
export const canRecordPayments = (user: UserLike): boolean => BILLING_ROLES.includes(user?.role) || can(user?.role, "billing", "edit");
export const canManageFees = (user: UserLike): boolean => FEE_MANAGER_ROLES.includes(user?.role) || can(user?.role, "billing", "manage");

export const jsonError = (message: string, status: number): Response =>
  Response.json({ success: false, message }, { status });

export const isValidId = (id: unknown): boolean => !!id && mongoose.isValidObjectId(id);

export async function verifyBillingUser(req: Request): Promise<AuthResult> {
  try {
...
export async function verifySchoolScope(user: UserDocument, schoolId: Id): Promise<boolean> {
...
export async function checkSchoolAccess(
  user: UserDocument,
  schoolId: Id,
  { requireFeeManager = false, requireRecorder = false }: SchoolAccessOptions = {}
...
export async function authorizeSchool(req: Request, schoolId: Id, options?: SchoolAccessOptions): Promise<AuthResult> {
...
export function generateReceiptNo(date: Date = new Date()): string {
...
export function parsePaymentInput(body: Record<string, unknown>): PaymentDetails | { error: string } {
  const method: PaymentMethod = PAYMENT_METHODS.includes(body.method as PaymentMethod)
    ? (body.method as PaymentMethod)
    : "cash";
  const paidAt = body.paidAt ? new Date(body.paidAt as string | number) : new Date();
...
export async function syncMissingBills(schoolId: Id, academicSession: string, term: string, userId: ObjectId): Promise<number> {
...
export const isEarlierTerm = (session: string, term: string, thanSession: string, thanTerm: string): boolean =>
  session < thanSession || (session === thanSession && TERM_ORDER[term] < TERM_ORDER[thanTerm]);

export const termLabel = (term: string | null | undefined, session: string | null | undefined): string =>
 ...
 */
export async function carryBalancesInto(
  bills: StudentBillDocument[],
  userId: ObjectId
): Promise<{ carried: number; amount: number }> {
...
export async function releaseArrears(bills: StudentBillDocument[], userId: ObjectId): Promise<void> {
...
export const isCarriedForward = (bill: { carriedForward?: { amount?: number | null } | null }): boolean => (bill.carriedForward?.amount || 0) > 0;

export const carriedForwardMessage = (bill: { carriedForward?: { term?: string | null; academicSession?: string | null } | null }): string =>
...
export async function applyStructureToBills(structure: FeeStructureDocument, userId: ObjectId): Promise<number> {
  ...
  for (const bill of bills) {
    ...
    if (oldItemsKey !== itemsKey) {
      ...
      // Mongoose converts the plain array into subdocuments on assignment.
      bill.items = newItems as typeof bill.items;
      bill.activity.push({
        action: "fees-updated",
