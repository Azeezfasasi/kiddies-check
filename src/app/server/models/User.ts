...
userSchema.methods.incLoginAttempts = async function () {
  ...
  const updates: mongoose.UpdateQuery<unknown> = { $inc: { loginAttempts: 1 } };
...
/** Plain field shape of a User (as returned by `.lean()`). */
export type UserFields = InferSchemaType<typeof userSchema>;

/** Instance methods attached via `userSchema.methods` above. */
export interface UserMethods {
  matchPassword(enteredPassword: string): Promise<boolean>;
  changedPasswordAfter(JWTTimestamp: number): boolean;
  getPasswordResetToken(): string;
  getEmailVerificationToken(): string;
  generateRegistrationOTP(): string;
  verifyRegistrationOTP(otp: string): boolean;
  isAccountLocked(): boolean;
  incLoginAttempts(): Promise<mongoose.UpdateWriteOpResult>;
  resetLoginAttempts(): Promise<mongoose.UpdateWriteOpResult>;
  getPublicProfile(): Record<string, unknown>;
  canAccessSchool(schoolId: { toString(): string }): boolean;
  getAccessibleSchools(): Promise<mongoose.Types.ObjectId[]>;
}

/** A loaded User document (as returned by `findById` etc.). */
export type UserDocument = HydratedDocument<UserFields, UserMethods>;

/** Statics attached via `userSchema.statics` above. */
export interface UserModel extends mongoose.Model<UserFields, Record<string, never>, UserMethods> {
  findByEmail(email: string): mongoose.Query<UserDocument | null, UserDocument>;
  findByRole(role: string): mongoose.Query<UserDocument[], UserDocument>;
}

// The cast only adds the method/static types above; the model itself is
// registered exactly as before.
export default defineModel("User", userSchema) as unknown as UserModel;
