...
export const formatNaira = (value) => naira.format(Number(value) || 0);
...
export const billTermLabel = (bill) => `${TERM_LABELS[bill.term] || bill.term} ${bill.academicSession}`;
...
const layout = ({ schoolName, heading, accent, body, portalUrl }) => `<!doctype html>
...
export function paymentReceiptEmail({ schoolName, guardianName, studentName, className, bill, payment, portalUrl }) {
...
export function feeReminderEmail({ schoolName, guardianName, studentName, className, bill, message, portalUrl }) {
  const overdue = bill.dueDate && new Date(bill.dueDate) < new Date();
  const lines = [
    ...(bill.arrears || []).map((a) => [`Balance brought forward (${TERM_LABELS[a.term] || a.term} ${a.academicSession})`, a.amount]),
    ...(bill.items || []).map((i) => [i.name, i.amount]),
    ...(bill.discount?.amount > 0 ? [["Discount", -bill.discount.amount]] : []),
  ];
  const body = `
    <p style="margin:0 0 16px;font-size:15px;color:#111827">Dear ${escapeHtml(guardianName || "Parent/Guardian")},</p>
    <p style="margin:0 0 20px;font-size:15px;color:#374151;line-height:1.5">
