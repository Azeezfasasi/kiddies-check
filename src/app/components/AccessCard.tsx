                ...
                <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-lg overflow-hidden border-2 border-white">
                  {student.picture ? (
                    <img
                      ...
                      onError={(e: React.SyntheticEvent<HTMLImageElement> & { target: HTMLImageElement }) => { e.target.style.display = 'none'; (e.target.parentNode as HTMLElement).innerHTML += `<span class='text-blue-600 font-bold text-2xl'>👤</span>`; }}
                    />
                  ) : (
