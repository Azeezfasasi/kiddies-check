...
import { unsubscribeFromNewsletter } from '@/app/server/controllers/newsletterController';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (result.success) {
      ...
      return new NextResponse(errorHTML((result as { error?: string }).error || 'Could not unsubscribe'), {
        headers: { 'Content-Type': 'text/html' },
        status: 400,
      });
    }
