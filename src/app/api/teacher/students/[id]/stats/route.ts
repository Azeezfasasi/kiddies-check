...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const assessments = await Assessment.find({
      student: id,
      school: schoolId,
    }).populate<{ subject: PopulatedNamed | null }>("subject", "name");

    // Group assessments by subject and calculate average scores.
    const subjectMap: Record<string, { subject: string; scores: number[]; maxScore: number }> = {};
    ...
    const attendanceMap: Record<string, { month: string; present: number; absent: number; late: number }> = {};
    const monthNames = [
      "Jan",
      "Feb",
      "Mar",
