...
import type { PopulatedNamed, PopulatedStudent } from "@/types/populated";
...
export async function GET(req: NextRequest) {
  try {
    ...
    } else if (!(await canAccessSchool(user, schoolId, "view"))) {
      return NextResponse.json({ success: false, message: "Access denied" }, { status: 403 });
    }
    ...
    const reportCards = await ReportCard.find(query)
      .populate<{ student: PopulatedStudent }>({ path: "student", select: "firstName lastName enrollmentNo" })
      .populate<{ class: PopulatedNamed }>({ path: "class", select: "name level section" })
